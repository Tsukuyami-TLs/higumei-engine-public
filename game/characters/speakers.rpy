# Declare characters

define narrator = Character(None, ctc="ctcArrow", ctc_position="fixed")
define dlanor = Character("Dlanor", ctc="ctcArrow", ctc_position="fixed")
define beatrice = Character("Beatrice", ctc="ctcArrow", ctc_position="fixed")
define erika = Character("Erika Furudo", ctc="ctcArrow", ctc_position="fixed")
define kanon = Character("Kanon", ctc="ctcArrow", ctc_position="fixed")
define jessica = Character("Jessica Ushiromiya", ctc="ctcArrow", ctc_position="fixed")
define shannon = Character("Shannon", ctc="ctcArrow", ctc_position="fixed")
define nao = Character("Nao Houtani", ctc="ctcArrow", ctc_position="fixed")
define shion = Character("Shion Sonozaki", ctc="ctcArrow", ctc_position="fixed")
define mion = Character("Mion Sonozaki", ctc="ctcArrow", ctc_position="fixed")
define kazuho = Character("Kazuho Kimiyoshi", ctc="ctcArrow", ctc_position="fixed")
define rika = Character("Rika Furude", ctc="ctcArrow", ctc_position="fixed")
define satoko = Character("Satoko Houjou", ctc="ctcArrow", ctc_position="fixed")
define miyuki = Character("Miyuki Akasaka", ctc="ctcArrow", ctc_position="fixed")
define hanyuu = Character("Hanyuu", ctc="ctcArrow", ctc_position="fixed")
define rena = Character("Rena Ryuugu", ctc="ctcArrow", ctc_position="fixed")
define ange = Character("Ange Ushiromiya", ctc="ctcArrow", ctc_position="fixed")
define akasaka = Character("Mamoru Akasaka", ctc="ctcArrow", ctc_position="fixed")
define kasai = Character("Tatsuyoshi Kasai", ctc="ctcArrow", ctc_position="fixed")
define keiichi = Character("Keiichi Maebara", ctc="ctcArrow", ctc_position="fixed")
define battler = Character("Battler Ushiromiya", ctc="ctcArrow", ctc_position="fixed")

define chara_placeholder = Character("placeholder_dispname", ctc="ctcArrow", ctc_position="fixed", dynamic=True)
define placeholder_dispname = "????"
